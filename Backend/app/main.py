...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"Warning: feature cache warm-up failed on startup: {exc}")
    ...
    try:
        ...
        def _bg_warm_releases():
            try:
                ...
                print(f"Warning: release provider cache warm-up failed: {e}")

        Thread(target=_bg_warm_releases, daemon=True).start()
    except Exception as exc:
        print(f"Warning: could not start release provider warm-up: {exc}")

    yield


app = FastAPI(title="Omify API", version="0.1.0", lifespan=lifespan)

# Frontend is served separately (static files / different port),
# so CORS must be open to it during local dev.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
